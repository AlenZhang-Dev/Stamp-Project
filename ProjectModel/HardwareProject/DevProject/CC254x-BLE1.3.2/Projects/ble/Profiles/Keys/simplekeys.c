  ...
  switch ( param )
  {
    case SK_KEY_ATTR:
      if ( len == sizeof ( uint8 ) ) 
      {
        ...
        /**
        系统处理，无需关注
        各参数含义猜测：skConfig：Key Pressed Characteristic Configs 按键属性的配置文件
        &skKeyPressed : Key Pressed State Characteristic 按键按下的状态属性  对应上一级按下的按键
        simplekeyAtrTb1:Profile Attributes - Table 存储按键的属性表
        Gatt_num_attts :The number of attribute records in a given attribute table 属性表数量的记录
        Invalid_task_id:判断数据是否有效     
        */
      }
      else
      {
