...
#define SBP_PERIODIC_EVT_PERIOD                   1500
...
static uint8 advertData[] =
{
  ...
  // mode (adverdiscoverabletises for 30 seconds at a time) instead of general
  //  mode (advertises indefinitely)
  0x02,   // length of this data
  GAP_ADTYPE_FLAGS,
...
#define DEFAULT_DEVICE_NAME  "Stamp0001"
...
static void performPeriodicTask( void );//周期性事件
...
uint16 SimpleBLEPeripheral_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  if ( events & SBP_START_DEVICE_EVT )
  {
    ...
    // Set timer for first periodic event   // 这里暂时不启动定时器， 而在后面连接上主机后才启动定时器， 这样连接才能更稳定， 否则不稳定， 单片机坑多呀 ---阿莫
...
static void simpleBLEPeripheral_HandleKeys( uint8 shift, uint8 keys )
{
  ...
  if ( keys & HAL_KEY_SW_6 )
  {
    ...
    //按键次数检测函数，按键次数 > 0 即可以继续操作按键；否则上锁操作
    if(Key_Value > 0)
    {
      Key_Value--;
    }
  ...
  //哪个按键按下，此函数用于确定哪个按键进行notify
  
  
  
  //SK_KEY-ATTR  Profile Attribute value
  //SK_KEYS  哪个按键按下
  
  //剩余授权次数减一
  
  //读取CHar2存储的次数，减一，写回char2
    ...
    case GAPROLE_CONNECTED:
      {
          ...
          osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_EVT, 3000 );  // 这一句很重要，也就是说，连接上之后，才开启定时器读取传感器数据， 否则会造成连接不稳定， 单片机就是坑多，我们要小心，要花费很多时间去调试才能稳定下来  
          HalLedSet( (HAL_LED_1 | HAL_LED_2 | HAL_LED_3), HAL_LED_MODE_OFF );  //  连接上后把灯全部关闭    
          HalLedBlink( (HAL_LED_1 | HAL_LED_2 | HAL_LED_3),  3, 50, 200);      // 连接上后灯闪3次
...
//此函数处理simpleProfileChangeCB函数需要调用的数据
uint8 simpleProfileReadConfig(uint16 uuid, uint8 *newValue)
{
    ...
    switch ( uuid )
    {
      ...
      {
        // 注意到 SensorValue 已经在定时函数中1s更新一次了， 所以这里直接送数据即可
      ...
      case SIMPLEPROFILE_CHAR7_UUID://设备名称写入
  ...
  switch( paramID )
  {
    case SIMPLEPROFILE_CHAR1:  // 收到 CHAR1 的数据
      ...
      if(newValue[0] == 0x11)
      {
         ...
         //接受成功在串口输出相关数据
     //    NPI_WriteTransport("led1 OPEN",returnBytes);  
         //使用未封装串口收发测试
      //   HalUARTWrite(0, "Hello", returnBytes);
         
        //Test：改变HalLedSet设置，其值发生变化，证明此函数为主机对从机处理的函数。
         
      }
      else if(newValue[0] == 0x10)
      {
         HalLedSet( (HAL_LED_3), HAL_LED_MODE_OFF );      
    //     NPI_WriteTransport("led1 CLOSE",returnBytes);                           
     ...
     //    NPI_WriteTransport(newValue,returnBytes);    
      ...
      // 继电器控制   // P1.3 控制继电器的开关
      // 通过此函数进行驱动板的控制，需要找到4个不相互影响的I/O口进行驱动，同时通过KeyValue值对驱动板上电与否进行控制
      //function：手动控制驱动板上电与否
    ...
    case SIMPLEPROFILE_CHAR5: // 收到 CHAR5 的数据 设置温度更新时间
      ...
      if(returnBytes == 4)
      {
        ...
        osal_snv_write(0x80, sizeof(SYS_CONFIG), &sys_config);    // 修改sys_config中的time_ms数据并将其写入nv中
      }
    ...
    case SIMPLEPROFILE_CHAR6:  // 这个是我们添加char6， 用于做串口透传 与从机 notify 数据到主机 很合适
...

      {
        ...
        // 通过串口透传出去，达到透传目的，透传就是使用 CHAR6
        NPI_WriteTransport(newValue, returnBytes);//寻找newvalue的值
        //Android端的接受
    ...
    case SIMPLEPROFILE_CHAR7: // char7 特性为设备名称的写入
      SimpleProfile_GetParameter( SIMPLEPROFILE_CHAR7, &newValue, &returnBytes);
      osal_memset(sys_config.name, 0, sizeof(sys_config.name));//初始化name值
      osal_memcpy(sys_config.name, newValue, returnBytes);//写入newvalue值
      osal_snv_write(0x80, sizeof(SYS_CONFIG), &sys_config);    // 写所有参数
    ...
    case SIMPLEPROFILE_CHAR8: // 收到 CHAR8 的数据   用于设置最高和最低温度报警
    ...
    case SIMPLEPROFILE_CHARA: // // pwm 
      ...
      if(returnBytes == 4)
      {
          sys_config.pwm[0] = newValue[0]; sys_config.pwm[1] = newValue[1];  sys_config.pwm[2] = newValue[2];  sys_config.pwm[3] = newValue[3];
          if(sys_config.pwm[0] == 0 && sys_config.pwm[1] == 0 &&  sys_config.pwm[2] == 0 &&  sys_config.pwm[3] == 0)
          {   osal_pwrmgr_device( PWRMGR_BATTERY ); } // 无pwm 输出了， 可以睡眠   
          else
          {   osal_pwrmgr_device( PWRMGR_ALWAYS_ON ); }  // 有 pwm 输出了， 不可以睡眠   
